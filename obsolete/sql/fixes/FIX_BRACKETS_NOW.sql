-- FIX BRACKETS - Run this in Supabase SQL Editor to fix the "Failed to add players" error
...
-- Test the setup by showing table info
SELECT
  'Table created: ' || tablename as status
FROM pg_tables
WHERE schemaname = 'public'
AND tablename IN ('bracket_tournaments', 'bracket_players', 'bracket_matches')
ORDER BY tablename;