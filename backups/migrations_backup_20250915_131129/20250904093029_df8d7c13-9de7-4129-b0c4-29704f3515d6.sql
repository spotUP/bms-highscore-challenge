...
-- Restrict file types and sizes for game logos
CREATE POLICY "Restrict logo file types and sizes" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
  ...
  -- Limit file size to 5MB (approximate check via filename patterns)
  length(name) < 200
);

-- Update existing logo upload policy to be more restrictive
...
WITH CHECK (
  ...
  (storage.filename(name) ~* '\.(jpg|jpeg|png|gif|webp)$')
);
