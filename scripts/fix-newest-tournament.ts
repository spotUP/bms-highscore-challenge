...
async function fixNewestTournament() {
  console.log('=== Fixing Newest Tournament ===\n');

  try {
    // Get the newest double elimination tournament (the one with empty assignments)
    ...
    console.log(`Current matches: ${currentMatches?.length || 0}`);

    // Delete existing matches and rebuild with proper assignments
    await supabase
      .from('bracket_matches')
      .delete()
      .eq('tournament_id', tournament.id);
...
fixNewestTournament().catch(console.error);